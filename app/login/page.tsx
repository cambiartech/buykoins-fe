...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.auth.login(formData.email, formData.password, rememberMe)
      ...
      <div className="w-full lg:w-2/3 flex items-center justify-center px-4 sm:px-6 lg:px-12 py-12 relative z-10">
        <div className="w-full max-w-md">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  className={`w-4 h-4 rounded text-tiktok-primary focus:ring-tiktok-primary focus:ring-1 cursor-pointer ${
                    isDark
                  ...
                  disabled={isLoading}
                />
                <span className={`font-sequel ${
                  isDark ? 'text-white/60' : 'text-gray-600'
                }`}>Remember me</span>
              </label>
              <Link 
                href="/forgot-password" 
                className="text-tiktok-primary hover:underline font-sequel transition-colors"
              >
                Forgot password?
              </Link>
            </div>
          ...
          <p className={`text-sm text-center mt-6 font-sequel ${
            ...
            <Link href="/signup" className="text-tiktok-primary hover:underline font-semibold transition-colors">
              Sign up
            </Link>
          </p>
        </div>
