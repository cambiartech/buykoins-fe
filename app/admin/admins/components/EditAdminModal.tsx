...
import { X, ArrowRight, ArrowLeft, Check, Lock } from '@phosphor-icons/react'
...
export function EditAdminModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only include permissions if role is admin
      if (formData.role === 'admin') {
        payload.permissions = formData.permissions
      }

      const response = await api.admin.updateAdmin(admin.id, payload)
      ...
      if (error instanceof ApiError) {
        const errorMsg = error.message || 'Failed to update admin'
        setErrors({ general: errorMsg })
        toast.error(errorMsg)
      } else {
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center space-x-2 px-4 py-2.5 rounded-lg border-2 font-sequel transition-colors ${
                      isDark
                        ? 'border-white/20 hover:border-tiktok-primary/50 bg-white/5 hover:bg-white/10 text-white/80'
                        : 'border-gray-300 hover:border-tiktok-primary/50 bg-white hover:bg-gray-50 text-gray-700'
                    }`}
                  >
                  ...
                  <p className={`mt-1 text-xs font-sequel ${theme.text.muted}`}>
                    Click to change password (requires OTP verification)
                  </p>
                </div>
              </div>
            )}
