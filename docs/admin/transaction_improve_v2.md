...
```typescript
{
  ...
  amountInNgn: number | null;    // NGN equivalent (for withdrawals/payouts)
  exchangeRate: number | null;   // Exchange rate used (USD to NGN)
  processingFee: number | null;  // Processing fee in NGN
  netAmount: number | null;      // Net amount in NGN (after fees)
...
**Finance Fields (for withdrawals/payouts):**
- **amountInNgn**: NGN equivalent at the time of withdrawal (null for credits)
- **exchangeRate**: Exchange rate used for conversion (null for credits)
- **processingFee**: Processing fee charged in NGN (null for credits)
- **netAmount**: Net amount paid to user in NGN (amountInNgn - processingFee, null for credits)
