...
## ⚠️ Important: Balance System

**CRITICAL**: The platform uses **two separate balances**:

1. **Earnings** (`earnings`): TikTok earnings and platform credits
   - Admin-approved credit requests → **earnings**
   - Widget deposits → **earnings**
   - Payouts/withdrawals → Deduct from **earnings**

2. **Wallet** (`wallet`): Spending balance for digital purchases
   - Paystack payments → **wallet**
   - Transfers from earnings → **wallet**
   - Card funding → Deducts from **wallet**

**Remember**: 
- ✅ Credit request approvals go to **earnings** (NOT wallet)
- ✅ Paystack payments go to **wallet** (NOT earnings)
- ✅ This is the FIRST time we're implementing wallet functionality
